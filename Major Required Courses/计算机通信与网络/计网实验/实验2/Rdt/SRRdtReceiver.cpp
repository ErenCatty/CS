...
void SRRdtReceiver::receive(const Packet& packet) {
	...
	if (checkSum == packet.checksum) {
		...
		if (base == packet.seqnum) {//为基序列
			...
			packetWaitingFlags[packet.seqnum % SEQLENGTH] = true;
			ReceivedPacket[packet.seqnum % SEQLENGTH] = packet;
			ReceivedPacket[packet.seqnum % SEQLENGTH].acknum = 0;
			while (packetWaitingFlags[base % SEQLENGTH])//取出Message，向上递交给应用层
			{
				Message msg;
				memcpy(msg.data, ReceivedPacket[base % SEQLENGTH].payload, sizeof(ReceivedPacket[base % SEQLENGTH].payload));
				pns->delivertoAppLayer(RECEIVER, msg);
				packetWaitingFlags[base++ % SEQLENGTH] = false;//释放缓存区
				packetWaitingFlags[NextSeqNum++ % SEQLENGTH] = false;//放入缓冲区
