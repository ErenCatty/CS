package hust.Computer_Networks;
...
public class MultiWebServer {
    ...
    public static void main(String[] args){
        while (true) {     //添加这个死循环使得html中的图片也被当成了一次http请求，所以会产生两次请求，当网页中的图片更多时，也许会产生更多http请求头，这可能不是我们想要的
            try {
                BufferedReader bufferedReader0 = new BufferedReader(
                        new InputStreamReader(new FileInputStream("D:\\IdeaProjects\\WebServer\\confi.txt"),"utf-8"));
                ...
                String for_url = bufferedReader0.readLine();
                ServerSocket serverSocket = new ServerSocket(confiPort,50, Inet4Address.getByName(confiIP));    //创建socket
                System.out.println("--------------------------------------");
                System.out.println("等待请求");
                Socket socket = serverSocket.accept();   //接收到客户端发来的socket，准备解析
                System.out.println("监听中...\r\n");

                InputStream inputStream = socket.getInputStream();  //注意：要从socket中读取信息，而不是serversocket
                ...
                if(url.equals(for_url)){
                    System.out.println("error:禁止访问\r\n");
                    outputStream = socket.getOutputStream();
                    StringBuffer error = new StringBuffer();
                    error.append("HTTP/1.1 403 Forbidden \r\n");
                    error.append("Content-Type:text/html \r\n");
                    error.append("Content-Length:22 \r\n").append("\r\n");
                    error.append("<h1>403 Forbidden</h1>");
                    outputStream.write(error.toString().getBytes());
                    System.out.println(error.toString());
                    outputStream.flush();
                }
                else if (new File(base_url + url).exists()) { //获取的文件存在
                    ...
                    StringBuilder append = result.append("HTTP/1.1 200 ok \r\n");
