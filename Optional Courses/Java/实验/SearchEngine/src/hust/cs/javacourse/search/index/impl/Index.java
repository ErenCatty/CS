...
/**
 * <pre>
 *      Index是AbstractIndex的具体实现，后者是内存中的倒排索引对象的抽象父类.
...
public class Index extends AbstractIndex implements FileSerializable {
    ...
    public void addDocument(AbstractDocument document) {
        ...
        TreeMap<AbstractTerm, Posting> map=new TreeMap<>();
        for(AbstractTermTuple termTuple: document.getTuples()){
            if(!map.containsKey(termTuple.term)){
                List<Integer> poslist=new ArrayList<>();
                poslist.add(termTuple.curPos);
                Posting posting=new Posting(document.getDocId(),1,poslist);
                map.put(termTuple.term,posting);
            }
            else{
                Posting posting1=map.get(termTuple.term);
                posting1.setFreq(posting1.getFreq()+1);
                List<Integer> poslist=posting1.getPositions();
                poslist.add(termTuple.curPos);
                posting1.setPositions(poslist);
            }
        }
